...
void Renderer::CreateGrassDescriptorSets() {
    ...
    std::vector<VkDescriptorSetLayout> layouts(scene->GetBlades().size(), modelDescriptorSetLayout);
...
void Renderer::RecordCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        clearValues[0].color = { 0.53f, 0.81f, 0.92f, 1.0f };  // Sky blue color
        clearValues[1].depthStencil = { 1.0f, 0 };
        renderPassInfo.clearValueCount = static_cast<uint32_t>(clearValues.size());
        renderPassInfo.pClearValues = clearValues.data();

        std::vector<VkBufferMemoryBarrier> barriers(scene->GetBlades().size() * 2);
        for (uint32_t j = 0; j < scene->GetBlades().size(); ++j) {
            ...
            barriers[j * 2].buffer = scene->GetBlades()[j]->GetNumBladesBuffer();
            barriers[j * 2].offset = 0;
            barriers[j * 2].size = sizeof(BladeDrawIndirect);
        ...
        vkCmdPipelineBarrier(commandBuffers[i], VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT | VK_PIPELINE_STAGE_VERTEX_INPUT_BIT, 0, 0, nullptr, static_cast<uint32_t>(barriers.size()), barriers.data(), 0, nullptr);
